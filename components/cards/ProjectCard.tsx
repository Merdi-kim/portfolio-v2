...
const ProjectCard: FC<IProjectCardProps> = ({ right }) => {
  return (
    ...
    >
      <div className="h-full w-full absolute top-0 px-2 py-4 rounded-lg bg-gray-500 text-white opacity-90">
        <h3 className=" font-bold mt-[40%] mb-4 text-lg">
          Filecoin name service
        </h3>
        <p className="w-full mb-4 line-clamp-3">
        ...
        <div className="h-[2rem] flex justify-between w-full">
          <Link href={'/hi'}>
            <span className="h-11/12 w-5/12 cursor-pointer flex justify-center items-center rounded-lg bg-blue-600">
              Live
            </span>
          </Link>
          <Link href={'/yo'}>
